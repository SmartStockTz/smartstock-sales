    ...
    >
          ...
          <div class="flex-fill">
           ...
           padding: 4px; border-radius: 4px;width: 100%; height: 48px;"
            ...
            <mat-autocomplete #auto="matAutocomplete">
              <mat-option
                *ngFor="let option of customerState.customers | async"
          ...
          <div *ngFor="let cart of cartState.carts | async; let i = index">
            <mat-list-item>
              ...
              <mat-card-subtitle matLine>
                {{
                  channel === "whole"
                    ? "(" + cart.product.wholesaleQuantity + ") x "
                    : ""
                ...
                {{
                  cart.quantity *
                    (channel === "whole"
                      ? cart.product.wholesalePrice
                      : cart.product.retailPrice) | number
                }}
              </mat-card-subtitle>
      ...
      >
        ...
        <div class="checkout-container">
          <button
            [disabled]="(cartState.checkoutProgress | async) === true"
            (click)="checkout()"
          ...
          >
            <span style="float: left;">{{
              cartState.cartTotal | async | fedha | async
            }}</span>
            <mat-progress-spinner
              *ngIf="(cartState.checkoutProgress | async) === true"
              mode="indeterminate"
              diameter="25"
            ...
            <span
              style="float: right"
              *ngIf="(cartState.checkoutProgress | async) === false"
              >Checkout</span
            >
          </button>
          <button
...
export class CartComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    this.customerFormControl.setValue(
      this.cartState.cartOrder.value?.customer?.displayName
    );
  ...
  private discountListener(): void {
    this.discountFormControl.valueChanges
      .pipe(takeUntil(this.destroyer))
      .subscribe((value) => {
  ...
  checkout(): void {
    if (
      this.currentUser.id === "smartstock-hq" &&
      !this.cartState.selectedCustomer.value
    ) {
  ...
  openOptions() {
    this.dialog
      .open(DialogCashSaleCartOptionsComponent, {
      ...
      .subscribe((value) => {
        switch (value) {
          case "order":
            this.cartState
              .saveOrder(this.channel, this.currentUser)
              .then((_3) => {
