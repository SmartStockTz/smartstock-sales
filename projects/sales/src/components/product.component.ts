    ...
    <div>
      <div
        *ngIf="(deviceState.isSmallScreen | async) === false"
        ...
        [ngClass]="{ flipped: productIndex == flipped }"
      >
        <div
        ...
        >
          <mat-card
            class="front"
            style="text-align: center; width: 150px; height: 160px;"
            (click)="flip(productIndex)"
          >
            <mat-card-content>
      ...
      >
        <mat-list-item style="width: 100%;" (click)="openSheet(productIndex)">
          ...
          >
            Price :
            {{
              (isViewedInWholesale ? stock.wholesalePrice : stock.retailPrice)
                | fedha
                | async
            }}
          </mat-card-subtitle>
...
export class ProductComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.instock =
      (this.stock.stockable === true && this.stock.quantity > 0) ||
      this.stock.stockable === false;
    this.productState.flipped
      .pipe(takeUntil(this.fdestroy))
      .subscribe((value) => {
        this.flipped = value;
      });
  }
