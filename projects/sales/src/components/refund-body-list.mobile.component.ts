...
@Component({
  ...
  template: `
    <cdk-virtual-scroll-viewport [itemSize]="88" style="height: 90vh">
      <mat-nav-list>
        <mat-list-item (click)="refund(sale)" *cdkVirtualFor="let sale of dataSource.connect() | async">
          <span matLine>{{sale.product}}</span>
          <mat-card-subtitle matLine>Sold at : {{sale.timer | muda }}</mat-card-subtitle>
          <mat-card-subtitle matLine>
            Return( amount: {{sale?.refund?.amount | fedha | async}}, quantity: {{sale?.refund?.quantity}} )
          </mat-card-subtitle>
...
export class RefundBodyListMobileComponent implements OnInit, OnDestroy, AfterViewInit {
