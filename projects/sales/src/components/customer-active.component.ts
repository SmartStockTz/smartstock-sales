...
@Component({
  ...
  template: `
    <div *ngIf="currentUser" style="display: inline-block">
      <div style="background: green; border-radius: 20px; width: 10px; height: 10px;display: inline-block" *ngIf="verified===true"></div>
      <div style="background: red; border-radius: 20px; width: 20px; height: 10px; display: inline-block" *ngIf="verified===false"></div>
    </div>
  `,
  styleUrls: []
})
