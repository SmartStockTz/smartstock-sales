  ...
  template: `
    <div id="cart_view" [ngClass]="(deviceState.isSmallScreen | async)===true?'cart-mobile':'cart'">
      <mat-toolbar class="mat-elevation-z3" style="z-index: 10000">
        <span [matBadge]="invoiceCartState.cartTotalItems | async" matBadgeOverlap="false">Cart</span>
          ...
          <div class="flex-fill">
            ...
            <mat-autocomplete #auto="matAutocomplete">
              <mat-option *ngFor="let option of suppliersDatasource.connect() | async"
                          [value]="option.displayName?option.displayName: option.firstName"
      ...
      <div style="padding-bottom: 500px">
        <mat-list>
          <div *ngFor="let cart of invoiceCartState.carts | async; let i=index">
            <mat-list-item matTooltip="{{cart.stock.product}}">
              <button (click)="invoiceCartState.removeItemFromCart(i)" matSuffix mat-icon-button>
                <mat-icon color="warn">delete</mat-icon>
              </button>
              <h4 matLine class="text-truncate">{{cart.stock.product}}</h4>
              <mat-card-subtitle matLine>
                {{cart.quantity | number}} {{cart.stock.unit}} @ {{cart.amount | number}}
                = {{cart.quantity * cart.amount | number}}
              </mat-card-subtitle>
              <div class="d-flex flex-row" matLine>
                <button color="primary" (click)="invoiceCartState.decrementCartItemQuantity(i)" mat-icon-button>
                  <mat-icon>remove_circle</mat-icon>
                </button>
                <button color="primary" (click)="invoiceCartState.incrementCartItemQuantity(i)" mat-icon-button>
      ...
      <div style="padding: 8px 8px 16px 8px;bottom: 0;width: 100%;position: absolute;background-color: white;z-index: 1000;">
        ...
        <div class="checkout-container">
          <button [disabled]="(invoiceState.addInvoiceProgress | async)===true"
                  (click)="checkout()"
                  style="width: 100%;text-align:left;height: 48px;font-size: 18px" color="primary"
                  mat-flat-button>
            <span style="float: left;">{{invoiceCartState.cartTotal | async | number }}</span>
            <mat-progress-spinner *ngIf="(invoiceState.addInvoiceProgress | async)===true"
                                  mode="indeterminate"
                                  diameter="25"
                                  style="display: inline-block; float: right">
            </mat-progress-spinner>
            <span style="float: right" *ngIf="(invoiceState.addInvoiceProgress | async)===false">Record</span>
          </button>
...
export class InvoiceCartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.invoiceCartState.carts.pipe(
      takeUntil(this.destroyer)
    ).subscribe(value => {
      if (Array.isArray(value)) {
  ...
  checkout(): void {
    if (!this.invoiceCartState.selectedCustomer.value && this.supplierFormControl.value && this.supplierFormControl.value !== '') {
