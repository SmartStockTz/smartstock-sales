...
export class CartService {
  ...
  private static async withWorker(fn: (cartWorker: CartWorker) => Promise<any>): Promise<any> {
    let nativeWorker: Worker;
    try {
      nativeWorker = new Worker(new URL('../workers/cart.worker', import .meta.url));
  ...
  async findTotal(carts: CartItemModel[], channel: string, discount: any): Promise<number> {
    return CartService.withWorker(cartWorker => cartWorker.findTotal(carts, channel, discount));
  }

  async addToCart(carts: CartItemModel[], cart: CartItemModel) {
    const shop = await this.userService.getCurrentShop();
    return CartService.withWorker(cartWorker => cartWorker.addToCart(carts, cart));
  }
  ...
  async checkout(
    ...
    const salesToSave: SalesModel[]
      = await CartService.withWorker(cartWorker => cartWorker.getSalesBatch(carts, channel, discount, customer, user));
  ...
  async printCart(carts: CartItemModel[], channel: string, discount: number, customer: CustomerModel, printOnly: boolean): Promise<any> {
    ...
    const saleItems = await CartService.withWorker(cartWorker => cartWorker.cartItemsToSaleItems(carts, discount, channel));
    const salesItemForPrint = await CartService.withWorker(
      cartWorker => cartWorker.cartItemsToPrinterData(saleItems, customer, channel, discount, printOnly)
    );
