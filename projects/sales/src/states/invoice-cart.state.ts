...
export class InvoiceCartState {
  ...
  constructor(private readonly invoiceCartService: InvoiceCartService,
  ...
  addToCart(cart: InvoiceItemModel): void {
    this.invoiceCartService.addToCart(this.carts.value, cart).then(value => {
  ...
  findTotal(carts: InvoiceItemModel[]) {
    this.totalItems();
    this.invoiceCartService.findTotal(carts).then(value => {
